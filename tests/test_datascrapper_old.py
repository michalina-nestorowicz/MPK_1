...
class TestDataScrapper(fake_filesystem_unittest.TestCase):
    ...
    def test_check_correct_dir_in_project(self):
        ...
        results = dataset_scrapper_old.check_correct_dir_in_project('/home/mpk/',
    ...
    def test_txt_to_csv_no_file(self):
        ...
        with mock.patch('builtins.print') as mocked_print:
            dataset_scrapper_old.txt_to_csv('/home/mpk/', 'Wroclaw')
    ...
    def test_txt_to_csv_file_exists(self):
        ...
        original_json_load = dataset_scrapper_old.json.load
        dataset_scrapper_old.json.load = mock_json_load

        dataset_scrapper_old.open = MockFile

        dataset_scrapper_old.txt_to_csv('/home/mpk/', 'Wroclaw')

        self.assertTrue(os.path.exists('/home/mpk/csv_files/Wroclaw-routes.csv'))

        dataset_scrapper_old.json.load = original_json_load
        dataset_scrapper_old.open = open
...
""" NOT USED TEST
    def test_get_request_dict_has_url_key_return_request(self):
        test_dict = {'city_name': 'Test', 'url': 'test_url'}
        with mock.patch('dataset_scrapper.requests.get') as mock_get:
            dataset_scrapper.get_website(test_dict)
        mock_get.assert_called_with('test_url', timeout=5)

    def test_get_request_dict_incorrect_keys_raise_KeyError(self):
        test_dict = {'city_name': 'Test', 'test': 'test_url'}
        with mock.patch('dataset_scrapper.requests.get') as mock_get:
            with self.assertRaises(KeyError) as cm:
                dataset_scrapper.get_website(test_dict)
        mock_get.assert_not_called()
        the_exception = cm.exception
        self.assertEqual(the_exception.args, ('No url value in directory',))

    def test_get_request_dict_empty_raise_KeyError(self):
        test_dict = {}
        with mock.patch('dataset_scrapper.requests.get') as mock_get:
            with self.assertRaises(KeyError) as cm:
                dataset_scrapper.get_website(test_dict)
        mock_get.assert_not_called()
        the_exception = cm.exception
        self.assertEqual(the_exception.args, ('City dictionary is empty',))
"""